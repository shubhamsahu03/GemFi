...
import { convertToRuppeeUnit } from "../utils/format-currency";
...
export const summaryAnalyticsService = async (
  ...
  return {
    availableBalance: convertToRuppeeUnit(availableBalance),
    totalIncome: convertToRuppeeUnit(totalIncome),
    totalExpenses: convertToRuppeeUnit(totalExpenses),
    savingRate: {
    ...
    percentageChange: {
      ...percentageChange,
      previousValues: {
        incomeAmount: convertToRuppeeUnit(
          percentageChange.previousValues.incomeAmount
        ),
        expenseAmount: convertToRuppeeUnit(
          percentageChange.previousValues.expenseAmount
        ),
        balanceAmount: convertToRuppeeUnit(
...
export const chartAnalyticsService = async (
  ...
  const transaformedData = (resultData?.chartData || []).map((item: any) => ({
    date: item.date,
    income: convertToRuppeeUnit(item.income),
    expenses: convertToRuppeeUnit(item.expenses),
  }));
...
export const expensePieChartBreakdownService = async (
  ...
  const transformedData = {
    totalSpent: convertToRuppeeUnit(data.totalSpent),
    breakdown: data.breakdown.map((item: any) => ({
      ...item,
      value: convertToRuppeeUnit(item.value),
    })),
  };
